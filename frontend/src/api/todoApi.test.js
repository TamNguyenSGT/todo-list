...
describe("todoApi", () => {
  ...
  test("updateTodo should send PUT and return updated todo", async () => {
    ...
    expect(result).toEqual(mockResponse);
    expect(fetch).toHaveBeenCalledWith(`http://localhost:8080/api/todos/${id}`, expect.objectContaining({
      method: "PUT",
  ...
  test("deleteTodo should send DELETE and return message", async () => {
    ...
    expect(result).toEqual(mockResponse);
    expect(fetch).toHaveBeenCalledWith(`http://localhost:8080/api/todos/${id}`, { method: "DELETE" });
  });
