...
function App() {
  ...
  const saveEdit = async (task) => {
    ...
    try {
      ...
      setTasks(prev =>
        prev.map(t =>
          t.id === task.id ? { ...t, title: editingText } : t
        )
      );
          ...
          {filteredTasks.map((task) => (
            ...
            >
              ...
              {editingTaskId === task.id ? (
                <>
                  <input value={editingText} onChange={(e) => setEditingText(e.target.value)} style={{ ...styles.editInput, ...theme.input }} />
                  <button onClick={() => saveEdit(task)} style={theme.saveButton}>{t.save}</button>
                  <button onClick={() => setEditingTaskId(null)} style={theme.cancelButton}>{t.cancel}</button>
                </>
              ) : (
                <>
                  <span style={{ ...styles.taskText, textDecoration: task.completed ? 'line-through' : 'none', color: theme.text }}>
                    {task.title}
                  </span>
                  <button onClick={() => startEdit(task)} style={theme.editButton}>{t.edit}</button>
                  <button onClick={() => deleteTask(task.id)} style={theme.deleteButton}>{t.delete}</button>
                </>
              )}
            </motion.li>
          ))}
...
const lightTheme = {
  ...
  taskItem: { backgroundColor: '#f7f7f7' },
